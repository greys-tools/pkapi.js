...
module.exports = class Switch {
	...
	async patchTimestamp(token) {
		var data = await this.#api.patchSwitchTimestamp({switch: this.id, ...this, token});
		for(var k in data) if(KEYS[k]) this[k] = data[k];
		return this;
	}

	async patchMembers(token) {
		var data = await this.#api.patchSwitchMembers({switch: this.id, ...this, token});
		for(var k in data) if(KEYS[k]) this[k] = data[k];
		return this;
	}
